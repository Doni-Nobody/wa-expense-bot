...
async function appendExpense({ name, amount, category, note, sender }) {
  const now = new Date();
  // ✅ FIX: Tambahkan timeZone Asia/Jakarta supaya jam sesuai WIB
  const tanggal = now.toLocaleDateString('id-ID', { timeZone: 'Asia/Jakarta', day: '2-digit', month: '2-digit', year: 'numeric' });
...
async function getSummary(sender) {
  ...
  const now = new Date();
  // ✅ FIX: Gunakan timezone WIB untuk filter bulan ini
  const nowWIB = new Date(now.toLocaleString('en-US', { timeZone: 'Asia/Jakarta' }));
  const thisMonth = `${String(nowWIB.getMonth() + 1).padStart(2, '0')}/${nowWIB.getFullYear()}`;
